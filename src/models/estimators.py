...
def _xgboost_device() -> str:
    """Detecta GPU CUDA disponible; cae a CPU si no hay."""
    try:
        import subprocess
...
def build_xgboost():
    ...
    return XGBRegressor(
        ...
        min_child_weight=5,          # evita splits en grupos muy pequeños (rutas raras)
        n_jobs=-1,
        random_state=42,
        tree_method="hist",          # eficiente CPU y GPU en XGBoost 2.0+
        device=device,               # "cuda" si hay GPU, "cpu" si no
        objective="reg:squarederror",
        verbosity=0,                 # suprimido: el callback de train_model.py maneja el log
    )
