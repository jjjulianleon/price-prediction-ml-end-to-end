...
def evaluate_model(
    ...
    # Acumuladores increméntales — O(1) RAM, sin guardar todos los vectores completos.
    # Welford online para varianza (necesaria para R²) y MAE/MAPE por suma parcial.
    n = 0
    sum_sq_err = 0.0   # para RMSE
    sum_abs_err = 0.0  # para MAE
    sum_pct_err = 0.0  # para MAPE
    abs_err_list: list[float] = []  # solo para mediana — lista de MAEs por lote (mediana de medianas)
    # Para R²: necesitamos mean(y_true) y var(y_true). Usamos Welford online.
    y_mean = 0.0
    M2_y = 0.0  # varianza acumulada de y_true (Welford)
    total_rows = 0
    ...
    for window_idx, (window_start, window_end) in enumerate(
        ...
        for batch_idx, batch_df in enumerate(
            ...
            X_batch, y_batch = split_features_target(batch_df)
            X_transformed = materialize_matrix(preprocessor.transform(X_batch), matrix_format)
            y_arr = y_batch.to_numpy(dtype=np.float64)
            p_arr = np.asarray(model.predict(X_transformed), dtype=np.float64)

            # Acumuladores — sin guardar y_arr ni p_arr después de este bloque
            nb = len(y_arr)
            err = y_arr - p_arr
            sum_sq_err  += float(np.dot(err, err))
            sum_abs_err += float(np.sum(np.abs(err)))
            sum_pct_err += float(np.sum(np.abs(err) / np.maximum(np.abs(y_arr), 1e-8)))
            abs_err_list.append(float(np.median(np.abs(err))))  # mediana del lote

            # Welford online para varianza de y_true (R²)
            for yi in y_arr:
                n += 1
                delta = yi - y_mean
                y_mean += delta / n
                M2_y += delta * (yi - y_mean)

            total_rows += nb
            del X_batch, y_batch, X_transformed, y_arr, p_arr, err, batch_df
    ...
    rmse   = float(np.sqrt(sum_sq_err / total_rows))
    mae    = float(sum_abs_err / total_rows)
    med_ae = float(np.median(abs_err_list))   # mediana de medianas por lote
    mape   = float(sum_pct_err / total_rows) * 100
    var_y  = M2_y / total_rows if total_rows > 1 else 1.0
    r2     = float(1.0 - (sum_sq_err / total_rows) / (var_y if var_y > 0 else 1.0))
    if log_fn:
        elapsed = perf_counter() - start_ts
        log_fn(
            f"Completed {split_name} | rows={total_rows:,} | "
            f"rmse={rmse:.4f} | mae={mae:.4f} | med_ae={med_ae:.4f} | "
            f"mape={mape:.2f}% | r2={r2:.4f} | elapsed={elapsed:.1f}s"
        )
    return rmse, mae, med_ae, mape, r2
